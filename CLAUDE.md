...
- Releases/PyPI publish via release-please + PyPI trusted publishing. **One-time setup required before publishing works:** a `RELEASE_PLEASE_TOKEN` secret and a PyPI trusted-publisher + `pypi` GitHub environment. Until configured, release automation is inactive but does not block PRs.
